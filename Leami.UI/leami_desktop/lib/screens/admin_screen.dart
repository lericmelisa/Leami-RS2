...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return Scaffold(
        appBar: AppBar(title: Text('Administratorski profil')),
        body: Center(child: CircularProgressIndicator()),
      );
    }
    if (_error != null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Administratorski profil')),
  ...
  Future<void> _goToEdit() async {
    ...
    final res = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => AdminDetailsScreen(user: _user)),
    );
    if (res == true && mounted) {
